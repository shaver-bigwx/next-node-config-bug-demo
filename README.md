...
Demo reproduction for a recursion bug / interaction between `next` and `node-config` / `config`. 
`npm run build`/`npm run start` should fail with a `[RangeError: Maximum call stack size exceeded]`.
Comment out the inclusion of `publicRuntimeConfig` in `next.config.js` and rerun `npm run start` for the build to succeed. 
Appears to be related to https://github.com/vercel/next.js/commit/3c2a1959047f668dfffcd6344aeaa4acb531eccf#diff-7836d903b56ef6914ef5f3e8d1711c1d353d7b6ae4efefb0d2193897ee4fca85R1637 
